        ...
        clients: async (_, args) => {
            ...
            */
            ...
            const filteredClients = clients.filter((client) => {
                if (!filter || Object.keys(filter).lenght === 0) {
                    return true;
                }
                return Object.entries(filter).every(([field, value]) => {
                    // Object.entries retorna o meu filter como um array, onde os elementos desse array são um outro array com os elementos sendo o key/value do meu obejeto.
                    // field, nese caso é o name e value é o valor da propriedade name, ambos são string
                    if (client[field] === null || client[field] === undefined) {
                        return false;
                    }
                    if (typeof value === 'string') {
                        if (value.startsWith('%') && value.endsWith('%')) {
                            return client[field].includes(value.substr(1, value.lenght - 2))
                        }
                        if (value.startsWith('%')) {
                            return client[field].endsdWith(value.substr(1));
